...
def preprocessing_yee(features, labels):
    """
    not konw some preprocess pipeline needed to use
    :param features:
    :param labels:
    ...
    for i in range(3):
        # rgb_list.append(rgb_p[:,:,i] - tf.reduce_mean(rgb_p[:,:,i]))
        rgb_list.append(rgb_p[:,:,i])
    rgb_p = tf.stack(rgb_list, axis=-1)
    # --------------------------------------------------------------
    
    rgb_p = tf.pad(rgb_p, paddings, "CONSTANT") 
    noisy_p = tf.pad(noisy_p, paddings, "CONSTANT") 
    intensity_p = tf.pad(intensity_p, paddings, "CONSTANT") 
    gt_p = tf.pad(gt_p, paddings, "CONSTANT") 

    # rgb_p = tf.image.resize_bilinear(rgb_p, [384, 512, 3])
    # noisy_p = tf.image.resize_bilinear(noisy_p, [384, 512, 1])
...
def imgs_input_fn_yee(filenames, height, width, shuffle=False, repeat_count=1, batch_size=32):
    def _parse_function(serialized, height=height, width=width):
        features = \
            {
    ...
    dataset = dataset.map(_parse_function)
    dataset = dataset.map(
        lambda features, labels: preprocessing_yee(features, labels)
    )
